...
class Chronometer {
  int id;
  List<DateTime> startTimestamps;
  List<DateTime> stopTimestamps;
  DateTime lastReset;
  String name = "";
  ...
  Chronometer(this.id, this.startTimestamps, this.stopTimestamps, this.name, this.color, this.lastReset){
  ...
  static Chronometer fromJSONString(String jsonString, int id){
    ...
    return Chronometer(id, startTimestamps, stopTimestamps, name, color, lastReset);
  }
  ...
  Duration getRunningDuration(){
    ...
    if(isRunning){
      totalRunningDuration += DateTime.now().difference(lastReset);
    }
